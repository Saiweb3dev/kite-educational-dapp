...
const pinJSONtoIPFS = async (jsonData) => {
  ...
  const requestBody = {
    pinataMetadata: { name: jsonData.courseName }, // Metadata about the content being pinned
    pinataContent: jsonData, // The actual JSON data to be pinned
  };
  
  console.log("Environment variables loaded:", !!process.env.PINATA_API_KEY, !!process.env.PINATA_API_SECRET_KEY);
  ...
  try {
    ...
    const response = await axios.post(url, requestBody, {
      ...
      headers: {
        ...
        PINATA_API_SECRET_KEY: process.env.PINATA_API_SECRET_KEY, // Include the Pinata secret API key in the request headers
